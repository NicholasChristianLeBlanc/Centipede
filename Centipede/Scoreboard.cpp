...
void Scoreboard::Score(int score) {
	...
	if (score == 0) {
		for (int i = 0; i < 2; i++) {
			mScore.push_back(new Texture("0", "emulogic.ttf", 8 * Graphics::windowScale, mColor));
		...
		for (unsigned i = 0; i <= lastIndex; i++) {
			mScore.push_back(new Texture(str.substr(i, 1), "emulogic.ttf", 8 * Graphics::windowScale, mColor));
			mScore[i]->Parent(this);
...
void Scoreboard::SetColor(SDL_Color color) {
	mColor = color;
}